...
namespace DeLaSur.Backend.Application.Commands.Entrada.Insert
{
    public class InsertEntradaCommandHandler : IRequestHandler<InsertEntradaCommand, ResponseModel>
    {
        ...
        public InsertEntradaCommandHandler(IUnitOfWork unitOfWork)
        {
            this.unitOfWork = unitOfWork;
            entradaRepository = new EntradaRepository(unitOfWork.Connection, unitOfWork.Transaction);
            movimientoRepository = new MovimientoRepository(unitOfWork.Connection, unitOfWork.Transaction);
            materialBovedaRepository = new MaterialBovedaRepository(unitOfWork.Connection, unitOfWork.Transaction);
        }
        public async Task<ResponseModel> Handle(InsertEntradaCommand request, CancellationToken cancellationToken)
        {
            ...
            {
                var detalle = new DetalleMovimientoModel() { IdMaterial = item.IdMaterial, IdInventario = item.IdBoveda, IdTipoMaterial = (int)Enums.TipoMaterial.Gema, Cantidad = item.Cantidad };
